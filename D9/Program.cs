...
internal class Rope
{
    ...
    internal Rope(int knotsNumber)
    {
        ...
        KnotsPositionsHistory = Knots.Select((k, i) => new { Key = i + 1, Value = new List<Position>() }).ToDictionary(n => n.Key, n => n.Value);
    }
    ...
    private void ApplyStep(Direction direction)
    {
        ...
        Enumerable.Range(1, Knots.Count() - 1).ToList().ForEach(i => Knots[i].Pursue(Knots[i - 1], direction));
...
internal class Knot
{
    ...
    */
    ...
    internal void Pursue(Knot target, Direction direction)
    {
        if (!(target).Surrounding.Contains(this.Position) && target != this)
        {
            ...
            {
                case Direction.Right:
                    this.Position.X = target.Position.X - 1;
                    this.Position.Y = target.Position.Y + 0;
                    break;
                case Direction.Up:
                    this.Position.X = target.Position.X + 0;
                    this.Position.Y = target.Position.Y - 1;
                    break;
                case Direction.Left:
                    this.Position.X = target.Position.X + 1;
                    this.Position.Y = target.Position.Y + 0;
