...
{
    Monkeys.ForEach(monkey => monkey.EvaluateItemToThrow().ToList().ForEach(ia => Monkeys.First(m => m.MonkeyId == ia.ToMonkeyId).ItemsCarried.Enqueue(ia.item)));
...
{
    Monkeys2.ForEach(monkey => monkey.EvaluateItemToThrow().ToList().ForEach(ia => Monkeys2.First(m => m.MonkeyId == ia.ToMonkeyId).ItemsCarried.Enqueue(ia.item)));
    Console.WriteLine($"{i}         ");
    //Console.WriteLine($"          {i}         ");
    //Monkeys.ForEach(monkey => Console.WriteLine(monkey.ToString()));
}
...
internal class Monkey
{
    ...
    private readonly Func<Item, BigInteger> Operation;
    ...
    internal IEnumerable<ItemThrow> EvaluateItemToThrow()
    {
        ...
        while (ItemsCarried.Count > 0)
        {
            ...
            EvaluateNewWorryLevel(item);

            yield return item.Value % WorryTester == 0 ? new ItemThrow(item, MonkeyIdToSendIfTrue) : new ItemThrow(item, MonkeyIdToSendIfFalse);
        }
    }

    private void EvaluateNewWorryLevel(Item item)
    {
        if (DevWorryLevel > 1)
            item.Value = (BigInteger)Math.Round((decimal)(Operation(item) / DevWorryLevel), 0);
        else
            item.Value = Operation(item);
    }

    private Func<Item, BigInteger> ParseOperation(string operation)
        => (item) =>
        {
            var exp = operation.Split(" ");
            var right = exp[2] == "old" ? item.Value : BigInteger.Parse(exp[2]);
            switch (exp[1])
            {
                case "+":
